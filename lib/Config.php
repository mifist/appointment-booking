...
abstract class Config
{
    ...
    /**
     * Get ranks, categories, services and staff members for drop down selects
    ...
    public static function getCaSeSt()
    {
	    ...
	    // Ranks.
	    $rows = Entities\Rank::query()->fetchArray();
	    foreach ( $rows as $row ) {
		    $result['ranks'][ $row['id'] ] = array(
			    'id'   => (int) $row['id'],
			    'name' => Utils\Common::getTranslatedString( 'rank_' . $row['id'], $row['name'] ),
	    ...
	    $rows = Entities\Service::query( 's' )
                ->select( 's.id, s.category_id, s.rank_id, s.title, s.position, s.duration, MIN(ss.capacity_min) AS min_capacity, MAX(ss.capacity_max) AS max_capacity' )
